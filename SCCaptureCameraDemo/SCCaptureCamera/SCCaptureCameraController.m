...
#define CAMERA_MENU_VIEW_HEIGH  60  //menu
...
@property (nonatomic,strong) ALAssetsLibrary * ala;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.ala = [[ALAssetsLibrary alloc]init];
...
- (void)addCameraMenuView {
    ...
    self.scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, 320, 60)];
    [self.cameraMenuView addSubview:self.scrollView];
...
//    CGFloat eachW = SC_APP_SIZE.width / actionArr.count;
    ...
    [flashBtn setBackgroundImage:[UIImage imageNamed:@"flashing_off.png"] forState:UIControlStateNormal];
    flashBtn.frame = CGRectMake(150, 20, 30, 36);
    [flashBtn addTarget:self action:@selector(flashBtnPressed:) forControlEvents:UIControlEventTouchUpInside];
    [self.topContainerView addSubview:flashBtn];
    
    
    UIButton * swithBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    swithBtn.frame = CGRectMake(280, 30, 30, 26);
    [swithBtn setBackgroundImage:[UIImage imageNamed:@"switch_camera.png"] forState:UIControlStateNormal];
    [swithBtn setBackgroundImage:[UIImage imageNamed:@"switch_camera_h.png"] forState:UIControlStateSelected];
    [swithBtn addTarget:self action:@selector(switchCameraBtnPressed:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.topContainerView addSubview:swithBtn];
...
//            [SCCommon saveImageToPhotoAlbum:stillImage];//存至本机
            [self.ala saveImage:stillImage toAlbum:@"元子相册" completion:^(NSURL *assetURL, NSError *error) {
...
//        NSLog(@"%ld",self.cameraMenuView.subviews.count);
        NSUInteger number = self.scrollView.subviews.count-2;
//        NSLog(@"number =====%ld",number);
        
//        for (UIImageView * view in self.scrollView.subviews) {
//            NSLog(@"%@",view);
//        }
        UIImageView * photo = [[UIImageView alloc]initWithFrame:CGRectMake(10+10*number+40*number, 10, 40, 40)];
        photo.transform = CGAffineTransformMakeScale(0, 0);
        [self.imagesArray addObject:stillImage];
        photo.image = stillImage;
        photo.layer.masksToBounds = YES;
        photo.layer.cornerRadius = 6.0f;
        [self.scrollView addSubview:photo];
        [UIView animateWithDuration:0.2 animations:^{
            photo.transform = CGAffineTransformMakeScale(1.f, 1.f);
        }];
        if (number >= 6) {
            self.scrollView.contentSize = CGSizeMake(320+50+((number-6)*50), 0);
            self.scrollView.contentOffset = CGPointMake(50+((number-6)*50), 0);
